...
float2 mandlebulb(in float3 pos)
{
    float3 z = pos;
    float dr = 1.0;
    float r = 0.0;
    ...
    return float2(0.5 * log(r) * r / dr,0);
}
 float mb(float3 p, float power) {
    p.xyz = p.xzy;
    float3 z = p;
...
inline void PostEffect(RaymarchInfo ray, inout PostEffectOutput o)
{
    ...
    o.occlusion *= 1.0 - .5 * ray.loop / ray.maxLoop;
